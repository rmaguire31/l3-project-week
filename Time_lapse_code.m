...
FL2_u1 = FL_2(:,1) + (FL_2(:,2).*FL_2(:,6)) - 290;
FL3_u1 = FL_3(:,1) + (FL_3(:,2).*FL_3(:,6)) - 290;
...
FL_all = [FL1_c_mean;FL2_c_mean;FL3_c_mean];
FL_mean = mean(FL_all)
FL_all_error = [FL1_c_Aerror_mean;FL2_c_Aerror_mean;FL3_c_Aerror_mean];
FL_mean_error = mean(FL_all_error)/sqrt(length(FL_all_error))

%%
x = [1:10];
x_all = [1:10,1:10,1:10];
y_all_exp = [ex1_c;ex2_c;ex3_c];
error_all = [ex1_c_Aerror;ex2_c_Aerror;ex3_c_Aerror];

% figure
% % scatter(x_all, y_all);
% plot(x_all,y_all_exp,'x','color',[0.2 0.2 0.98],'linewidth',1.5); 
% % hold on
...
% % figure
hold on
y_all_FL = [FL1_c;FL2_c;FL3_c];
% % scatter(x_all, y_all);
% plot(x_all,y_all_FL,'^','color',[0.57 0 0],'linewidth',1.5); 
% % hold on
% % [ p, sp ] = linfitxy( x, y_all, 0, FL1_c_Aerror); %1 +/- STD lines
% % % A = p(1);
% % % B = p(2);
% % % x_lin = x;
% % % y_lin = A*x + B;
